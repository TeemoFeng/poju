...
class DocumentScript
{
    ...
    public static function showActionListDoc($controllersPath,$arrayControllers) {
        ...
        foreach ($controllerPathList as $pathKey=>$controllerPath) {
            ...
            $handle = @fopen($controllerPath, "r");
            $currentDocument = array();//当前的解析包
            $tagStart = false;//是否触发文档标记

            if ($handle) {
                while (!feof($handle)) {
                    $buffer = fgets($handle, 4096);
                    if (strpos($buffer, '/***') != false && strpos($buffer, '/**hide') === false && strpos($buffer, '/****') === false) {
                        $tagStart = true;
                    }
                    if ($tagStart) {
    ...
    public static function parseDoc($docStream) {
        ...
        $result['docParam']			= array();
        $result['docReturn']		= '';
        $result['docField']			= array();
        $result['docThrows']		= array();
        $result['jsonData']	        = '';
        $result['jsonDataInfo']	    = '';
        $result['docTodo']			= '';
        foreach ($docStream as $key=>$value) {
            ...
            $valueN = explode(' ', trim($value));

            $newValue = [];
            foreach ($valueN as $k => $v){
                if(!empty($v)){
                    $newValue[] = $v;
                }
            }
            ...
            if ('*' == $xing) {
                switch ($tag) {
                    ...
                    case '@license':
                        ...
                        $result['docLicense'] 	= ($con1);
                        $result['docRequestType'] = $con2;
                        $result['mklOldApi'] = $con3;
                        break;
                    case '@param':
                        $result['docParam'][$key]['type'] = $con1;
                        $result['docParam'][$key]['name'] = $con2;
                        list($result['docParam'][$key]['description'],$result['docParam'][$key]['isneed']) = explode('|', $con3);
                        break;
                    case '@para':
                        $result['docParam'][$key]['type'] = $con1;
                        $result['docParam'][$key]['name'] = $con2;
                    ...
                    case '@jsondata':
                        ...
                        unset($jsonValue[0]);
                        unset($jsonValue[1]);
                        $result['jsonData']	 = join(' ',$jsonValue);
                        break;
                    case '@jsondatainfo':
                        $jsonValue = $newValue;
                        unset($jsonValue[0]);
                        unset($jsonValue[1]);
                        $result['jsonDataInfo']	 = join(' ',$jsonValue);
                        break;
                    case '@todo':
                        $result['docTodo'] 	= $con1;
                        break;
