...
class Email
{
    ...
    public function __construct($options = [])
    {
        ...
        $this->mail->SMTPSecure
            = isset($securArr[$this->options['mail_verify_type']])
            ? $securArr[$this->options['mail_verify_type']] : '';
    ...
    public function send()
    {
        ...
        switch ($this->options['mail_type']) {
            case 1:
                //使用phpmailer发送
                $this->mail->setFrom($this->options['from'],
                    $this->options['from_name']);
                $this->mail->addAddress($this->options['to'],
            ...
            case 2:
                ...
                $result  = mail($this->options['to'], $this->options['subject'],
                    $this->options['body'], $headers);
